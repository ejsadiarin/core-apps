...
// Sessions
func (q *Queries) CreateSession(ctx context.Context, arg CreateSessionParams) (CoregatewaySession, error) {
	row := q.db.QueryRow(ctx, createSession, arg.UserID, arg.TokenHash, arg.ExpiresAt)
	var i CoregatewaySession
	err := row.Scan(
		&i.ID,
		&i.UserID,
		&i.TokenHash,
...
// Users
func (q *Queries) CreateUser(ctx context.Context, arg CreateUserParams) (CoregatewayUser, error) {
	row := q.db.QueryRow(ctx, createUser, arg.Email, arg.PasswordHash, arg.Role)
	var i CoregatewayUser
	err := row.Scan(
		&i.ID,
		&i.Email,
		&i.PasswordHash,
...
SELECT id, email, password_hash, role, created_at, updated_at FROM coregateway.users
WHERE id = $1 LIMIT 1
`

func (q *Queries) GetUser(ctx context.Context, id uuid.UUID) (CoregatewayUser, error) {
...
func (q *Queries) GetUserByEmail(ctx context.Context, email string) (CoregatewayUser, error) {
...
func (q *Queries) ListUsers(ctx context.Context) ([]CoregatewayUser, error) {
	rows, err := q.db.Query(ctx, listUsers)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []CoregatewayUser{}
	for rows.Next() {
		var i CoregatewayUser
		if err := rows.Scan(
			&i.ID,
...
func (q *Queries) UpdateUser(ctx context.Context, arg UpdateUserParams) (CoregatewayUser, error) {
